...
-- Ожидание загрузки локального игрока (критично для стабильного старта из Workspace)
while not botPlayer do
    botPlayer = playersService.LocalPlayer
    task.wait()
end
...
-- === Задние сенсоры и зигзаг-уклонение ===
local rearDetectionDist = 22
local rearPlayerDetected = false
local rearPlayerDist = 999
local zigzagTimer = 0
local zigzagDir = 1
local zigzagInterval = 0.35
local panicJumpCooldown = 0
...
local rearSensorAngles = {
    {name = "RearLeft",   angle = 140, dist = rearDetectionDist},
    {name = "RearRight",  angle = -140, dist = rearDetectionDist},
    {name = "DirectRear", angle = 180,  dist = rearDetectionDist},
}

-- Инициализация GUI (ResetOnSpawn = false позволяет окну не пропадать после смерти)
local gui = playerGui:FindFirstChild("Bot_Ultra_V16_5") or Instance.new("ScreenGui", playerGui)
gui.Name = "Bot_Ultra_V16_5"
gui.ResetOnSpawn = false

local alertLabel = gui:FindFirstChild("Alert") or Instance.new("TextLabel", gui)
alertLabel.Name = "Alert"; alertLabel.Size = UDim2.new(0, 400, 0, 60)
alertLabel.Position = UDim2.new(0.5, -200, 0.15, 0)
...
local function makeFrame(size, pos, color)
    ...
    f.Size = size; f.Position = pos; f.BackgroundColor3 = color; f.Visible = false
    Instance.new("UICorner", f).CornerRadius = UDim.new(0, 10)
    return f
end

local function makeButton(text, color, parent)
    local b = Instance.new("TextButton", parent); b.Size = UDim2.new(0.9, 0, 0, 32)
    b.BackgroundColor3 = color; b.Text = text; b.TextColor3 = Color3.new(1,1,1); b.Font = "GothamBold"; b.TextSize = 10
    Instance.new("UICorner", b).CornerRadius = UDim.new(0, 8)
    return b
end

local mainFrame = makeFrame(UDim2.new(0, 180, 0, 185), UDim2.new(1, -200, 0.4, 0), Color3.fromRGB(20, 20, 20))
...
local function createSlider(name, min, max, default, parent, callback)
    ...
    label.Size = UDim2.new(1, 0, 0, 20); label.BackgroundTransparency = 1; label.TextColor3 = Color3.new(1,1,1); label.Font = "GothamBold"; label.Text = name .. ": " .. default; label.TextSize = 10
    local bg = Instance.new("Frame", container)
    bg.Size = UDim2.new(0.9, 0, 0, 5); bg.Position = UDim2.new(0.05, 0, 0.7, 0); bg.BackgroundColor3 = Color3.new(0.1, 0.1, 0.1)
    Instance.new("UICorner", bg)
    local fill = Instance.new("Frame", bg); fill.Size = UDim2.new((default-min)/(max-min), 0, 1, 0)
    fill.BackgroundColor3 = Color3.fromRGB(60, 120, 255); Instance.new("UICorner", fill)
    local btn = Instance.new("TextButton", bg)
    btn.Size = UDim2.new(0, 12, 0, 12); btn.Position = UDim2.new((default-min)/(max-min), -6, 0.5, -6); btn.Text = ""
...
local function getKillerFromHit(hitInstance)
    ...
    for _, p in pairs(playersService:GetPlayers()) do
        if p ~= botPlayer and p.Character == model then
...
-- Автоматический перезапуск ссылок при смерти и спавне персонажа
local function onCharacterAdded(newChar)
    char = newChar
    hum = char:WaitForChild("Humanoid")
    hrp = char:WaitForChild("HumanoidRootPart")
    currentDirection = hrp.CFrame.LookVector
...
runService.Heartbeat:Connect(function()
    -- Безопасный выход из цикла, если персонаж отсутствует или мертв (предотвращает ошибки в Workspace)
    ...
    if flyToSpace or flyDown or emergencyActive or descending then
        ...
        -- Сканирование карты на наличие убийцы (игрок с ножом)
        ...
        -- Задние сенсоры обнаружения угрозы
        rearPlayerDetected = false
        rearPlayerDist = 999
        for _, cfg in ipairs(rearSensorAngles) do
            ...
            local killerHit = ray and getKillerFromHit(ray.Instance)
            if killerHit then
                rearPlayerDetected = true
                local d = (ray.Position - hrp.Position).Magnitude
                if d < rearPlayerDist then rearPlayerDist = d end
            end
            if laser then
                laser.Enabled = showRays
                laser.Attachment0.WorldPosition = origin
                if killerHit then
                    laser.Attachment1.WorldPosition = ray.Position
                    laser.Color = ColorSequence.new(Color3.new(1, 0.5, 0))
                elseif ray then
                    laser.Attachment1.WorldPosition = ray.Position
                    laser.Color = ColorSequence.new(Color3.new(0.3, 0.3, 0.3))
                else
                    laser.Attachment1.WorldPosition = origin + dir * cfg.dist
                    laser.Color = ColorSequence.new(Color3.new(0, 0.5, 1))
                end
            end
        end
        ...
        local threatDist = 999

        -- Обработка фронтальной угрозы
        if avoidPlayer and target and minDist < 40 then
            isUnderThreat = true
            threatDist = math.min(threatDist, minDist)
            local panicBoost = math.clamp((40 - minDist) * 1.5, 0, 60)
            speed = math.max(speed, baseSpeed + panicBoost)
            if minDist < 8 then autoSpaceActive = true; flyToSpace = true end
        end

        -- Обработка задней угрозы
        if rearPlayerDetected then
            local rearBoost = math.clamp((rearDetectionDist - rearPlayerDist) * 2.5, 0, 55)
            speed = math.max(speed, baseSpeed + rearBoost)
            if avoidPlayer then
                isUnderThreat = true
                threatDist = math.min(threatDist, rearPlayerDist)
            end
        end

        -- Текстовые паник-алерты
        if avoidPlayer then
            if target and minDist < 40 then
                alertLabel.Text = "🚨 ПАНИКА! УБИЙЦА: " .. math.floor(minDist) .. " 🚨"
                alertLabel.Visible = true
            elseif rearPlayerDetected then
                alertLabel.Text = "⚠️ СЗАДИ! ДИСТ: " .. math.floor(rearPlayerDist) .. " ⚠️"
                alertLabel.Visible = true
            else
                alertLabel.Visible = false
            end
        ...
        if moveToPlayer and target then
            wishDir = (target.Position - hrp.Position).Unit
        elseif avoidPlayer and target and minDist < 40 then
            wishDir = (hrp.Position - target.Position).Unit
        elseif avoidPlayer and rearPlayerDetected then
            wishDir = currentDirection
        end

        -- Зигзаг-уклонение и уклонительные панически прыжки
        if isUnderThreat then
            if tick() - zigzagTimer > zigzagInterval then
                zigzagDir = -zigzagDir
                zigzagTimer = tick()
                zigzagInterval = 0.25 + math.random() * 0.3
            end
            local zigzagAngle = math.clamp(35 + (30 - threatDist) * 1.2, 30, 65)
            wishDir = (CFrame.new(Vector3.zero, wishDir) * CFrame.Angles(0, math.rad(zigzagDir * zigzagAngle), 0)).LookVector
            if threatDist < 18 and tick() - panicJumpCooldown > 0.55 then
                hum:ChangeState(Enum.HumanoidStateType.Jumping)
                panicJumpCooldown = tick()
            end
        end

        -- Обнаружение препятствий и стен фронтальными усами
        ...
        for _, cfg in ipairs(visibleWhiskerAngles) do
            ...
            if laser then
                ...
                if ray then 
                    ...
                    laser.Attachment1.WorldPosition = origin + dir * cfg.dist
                    laser.Color = ColorSequence.new(Color3.new(0,1,0))
                end
            end
        end
        ...
        local urgency = isUnderThreat and math.clamp((30 - threatDist) / 30, 0, 1) or 0
        local lerpFactor = 0.15 + urgency * 0.12

        wallMemory = sideHits
        currentDirection = currentDirection:Lerp(wishDir, lerpFactor).Unit
        if autoJumpEnabled and tick() - lastJumpTime > jumpInterval then
            hum:ChangeState(Enum.HumanoidStateType.Jumping)
            lastJumpTime = tick()
        end

        -- Перемещение и разворот тела персонажа
        ...
        -- === ДОБАВЛЕНО: Автоматическое управление камерой ===
        local camera = workspace.CurrentCamera
        if camera then
            local cameraTargetPos = nil
            if target and (moveToPlayer or (avoidPlayer and minDist < 40)) then
                -- Если бот бежит к убийце или убегает от него, камера фокусируется прямо на убийце (как аимбот/слежка)
                cameraTargetPos = target.Position
            else
                -- В обычном режиме или режиме исследования камера плавно разворачивается туда, куда бежит бот
                cameraTargetPos = camera.CFrame.Position + Vector3.new(currentDirection.X, currentDirection.Y * 0.3, currentDirection.Z)
            end

            if cameraTargetPos then
                local targetCFrame = CFrame.lookAt(camera.CFrame.Position, cameraTargetPos)
                -- 0.15 — коэффициент плавности, камера следует за целью кинематографично и без дёрганий
